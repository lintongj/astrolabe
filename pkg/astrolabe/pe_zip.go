...
func ZipProtectedEntityToFile(ctx context.Context, srcPE ProtectedEntity, destFilePath string) (bytesWritten int64, err error) {
	...
	zipWriter, err = os.Create(destFilePath)
	if err != nil {
		err = errors.Errorf("Could not create file %s, err: %v", destFilePath, err)
		return
	}

	bytesWritten, err = io.Copy(zipWriter, reader)
	if err != nil {
...
func ZipProtectedEntityToWriter(ctx context.Context, pe ProtectedEntity, writer io.Writer) error {
	...
	if dataReader != nil {
		...
		dataWritten, err := io.CopyBuffer(peDataWriter, bufio.NewReaderSize(dataReader, bufSize), make([]byte, 1024*1024))
		if (err != nil) {
...
func UnzipFileToProtectedEntity(ctx context.Context, srcFilePath string, destPE ProtectedEntity) error {
	file, err := os.Open(srcFilePath)
	if err != nil {
		return errors.Errorf("Could not open srcFile %s, err = %v", srcFilePath, err)
	}

	fileInfo, err := file.Stat()
	if err != nil {
		return errors.Errorf("Got err %v getting stat for source file %v", err, srcFilePath)
	}

	srcPE, err := GetPEFromZipStream(ctx, file, fileInfo.Size())
	if err != nil {
		return errors.Errorf("Got err %v unzipping PE from source file %v", err, srcFilePath)
	}
